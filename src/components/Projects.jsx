...
const Projects = ({slides}) => {
    ...
    useEffect(()=>{
        if(current === 0){
            return setRepo(repoLink1)
        }if (current === 1) {
            return setRepo(repoLink2)
        } if(current === 2) {
  ...
  return (
    <>
        <section id='projects' className='w-[90%] sm:w-[30%] m-auto pt-8 pb-8 h-screen'>
            ...
            <div className="w-full overflow-hidden relative hover:scale-110 hover:duration-300 ease-out duration-300">
                ...
                {repo.map(el => {
                    return(
                            <div key={el.id} className='absolute z-40 top-0 buttonVerMas'>
                                <a  target='_blank' rel='noopener noreferrer' href={el.sitio}>
